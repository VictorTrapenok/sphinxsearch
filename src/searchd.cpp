//
// $Id: searchd.cpp 4660 2014-04-16 12:00:56Z tomat $
//
...
static const char * g_dSqlStmts[STMT_TOTAL] =
{
	...
	"select_dual", "show_databases", "show_balanstype", "create_plugin", "drop_plugin", "show_plugins", "show_threads"
};
...
void HandleMysqlShowDatabases ( SqlRowBuffer_c & tOut, SqlStmt_t & tStmt)
{
	tOut.HeadBegin ( 1 );
...
void HandleMysqlGetBalansType ( SqlRowBuffer_c & tOut, SqlStmt_t & )
{
	tOut.HeadBegin ( 1 );
	tOut.HeadColumn ( "SetBalansType" );
	tOut.HeadEnd();
...
void HandleMysqlInsert ( SqlRowBuffer_c & tOut, const SqlStmt_t & tStmt, bool bReplace, bool bCommit, CSphString & sWarning, const CSphString & sQuery  )
{
	...
	// get that index
	const ServedIndex_t * pServed = g_pLocalIndexes->GetRlockedEntry ( tStmt.m_sIndex );
	if ( !pServed ) // Используем распределённый индекс для вставки данных.
	{
            // extract index names 
            CSphVector<CSphString> dIndexNames;
...
{
        // REPLACE INTO dtest (id, `location`, `title`, `text`, `active`, `date`) VALUES ( 185191, ( 145430 ) , 'Тесто6', 'Твр', 1 , '1397711688');
	CSphString sError;
	// extract index names
	CSphVector<CSphString> dIndexNames;
	ParseIndexList ( tStmt.m_sIndex, dIndexNames );
	...
	ARRAY_FOREACH_COND ( i, tStmt.m_tUpdate.m_dAttrs, !bMvaUpdate )
	{
        ...
        printf("HandleMysqlUpdate::dIndexNames %d\n", dIndexNames.GetLength());
	ARRAY_FOREACH ( iIdx, dIndexNames )
	{
		const char * sReqIndex = dIndexNames[iIdx].cstr();
		const ServedIndex_t * pLocked = UpdateGetLockedIndex ( sReqIndex, bMvaUpdate );
...
public:
	...
	bool Execute ( const CSphString & sQuery, NetOutputBuffer_c & tOutput, BYTE & uPacketID, ThdDesc_t * pThd=NULL )
	{
		...
		case STMT_REPLACE:
			m_tLastMeta = CSphQueryResultMeta();
			m_tLastMeta.m_sError = m_sError;
			m_tLastMeta.m_sWarning = ""; // StatCountCommand ( SEARCHD_COMMAND_UPDATE );
		...
		case STMT_SHOW_BALANSTYPE:
			HandleMysqlGetBalansType ( tOut, *pStmt );
			return true;
